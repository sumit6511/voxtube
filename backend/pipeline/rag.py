...
def _build_prompt(question: str, source_comments: list[dict], history: list[dict] | None = None,
    ...
    return (
        ...
        f'Write an informative answer (roughly 3-5 sentences, or a couple of '
        f'short bullet points if the question naturally breaks into distinct '
        f'sub-points) that synthesizes the overall takeaway, using the '
        f'conversation so far for context if the question is a follow-up. '
        f'Use light markdown — **bold** for key numbers, percentages, and '
        f'topic names — but do not quote comments verbatim, list them one by '
