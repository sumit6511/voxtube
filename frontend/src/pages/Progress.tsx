...
const STAGES: { key: string; label: string }[] = [
  ...
  { key: 'preprocessing',   label: 'Neplish preprocessing'     },
  { key: 'analyzing',       label: 'Sentiment analysis'        },
  { key: 'toxicity',        label: 'Toxicity detection'        },
  { key: 'building_topics', label: 'Topic modeling (BERTopic)' },
  { key: 'building_rag',    label: 'Building search index'     },
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        setStatus(s)
        if (s.status === 'done') { clearInterval(intervalRef.current!); navigate(`/dashboard/${jobId}`) }
        if (s.status === 'failed') clearInterval(intervalRef.current!)
      } catch { /* keep polling */ }
    }
    poll()
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-16">
      <div className="animate-fade-up mb-10 text-center">
        ...
        {status?.video_title && (
          <p className="text-gray-400 text-sm font-body mt-1 max-w-sm truncate">{status.video_title}</p>
        )}
      </div>
      ...
      <div className="animate-fade-up w-full max-w-md card space-y-6">
        <div>
          ...
          <div className="h-1.5 bg-base rounded-full overflow-hidden">
            <div className="h-full bg-amber rounded-full transition-all duration-700 ease-out"
          ...
          {STAGES.filter(s => s.key !== 'done').map((stage, idx) => {
            ...
            return (
              <div key={stage.key}
                ...
                <span className={`text-sm font-body ${active ? '' : 'text-gray-400'}`}
                      style={active ? { color: 'var(--color-text)' } : {}}>
                  {stage.label}
                </span>
                {stage.key === 'fetching' && done && status?.comment_count ? (
                  <span className="ml-auto font-mono text-xs text-amber">{status.comment_count} comments</span>
                ) : null}
              </div>
            )
          })}
        ...
        {failed && status?.error_message && (
          <div className="bg-neg/5 border border-neg/20 rounded-lg px-4 py-3">
            ...
            <button onClick={() => navigate('/')} className="mt-2 text-xs text-gray-400 hover:text-white underline font-mono">
              ← Try again
            </button>
          </div>
        )}
      ...
      <p className="mt-6 text-xs font-mono text-gray-700">job: {jobId}</p>
    </div>
  )
}
