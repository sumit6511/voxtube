...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-16">
      ...
      <div className="animate-fade-up w-full max-w-lg card space-y-5" style={{ animationDelay: '80ms' }}>
        <div>
        ...
        <div>
          ...
          <input type="range" min={50} max={500} step={50} value={max}
            onChange={e => setMax(Number(e.target.value))} disabled={loading}
      ...
      <div className="animate-fade-up mt-8 flex flex-wrap justify-center gap-2" style={{ animationDelay: '160ms' }}>
      ...
      <div className="animate-fade-up mt-4 flex items-center gap-3" style={{ animationDelay: '220ms' }}>
        <Link to="/history"
          className="flex items-center gap-1.5 text-xs font-mono text-gray-500 hover:text-amber
                     border border-base-border hover:border-amber/40 px-4 py-2 rounded-full transition-all duration-200">
          <span className="text-amber">◷</span> History <ArrowRight size={12} />
        </Link>
        <Link to="/evaluate"
          className="flex items-center gap-1.5 text-xs font-mono text-gray-500 hover:text-amber
                     border border-base-border hover:border-amber/40 px-4 py-2 rounded-full transition-all duration-200">
          <span className="text-amber">⬡</span> Model Evaluation <ArrowRight size={12} />
        </Link>
      </div>
    </div>
  )
