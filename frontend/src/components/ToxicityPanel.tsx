...
export default function ToxicityPanel({ comments }: Props) {
  ...
  if (total === 0) return <p className="text-gray-600 font-mono text-sm">No comments to analyze.</p>
  ...
  for (const cat of CATEGORIES) {
    const sum = comments.reduce((acc, c) => acc + (safeParseScores(c.toxicity_json)[cat.key] ?? 0), 0)
  ...
  return (
    <div className="space-y-4 max-w-xl mx-auto">
          ...
          return (
            <div key={cat.key}>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <span className="font-mono text-xs" style={{ color: cat.color }}>{avg.toFixed(4)}</span>
              </div>
      ...
      <p className="text-xs text-gray-700 font-mono">Model: unitary/toxic-bert · threshold: 0.5</p>
    </div>
  )
}
