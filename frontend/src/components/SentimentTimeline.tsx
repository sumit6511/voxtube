...
interface TimePoint { label: string; positive: number; neutral: number; negative: number }
...
function buildTimeline(comments: Comment[]) {
  ...
  if (dated.length === 0) return { points: [] as TimePoint[], granularity: 'day' as Granularity, withDates: 0 }

  const ts = dated.map(c => new Date(c.published_at!).getTime())
  const span = (Math.max(...ts) - Math.min(...ts)) / 86_400_000
  const g: Granularity = span <= 14 ? 'day' : span <= 120 ? 'week' : 'month'

  const groups: Record<string, TimePoint> = {}
  for (const c of dated) {
    const key = getKey(new Date(c.published_at!), g)
    if (!groups[key]) groups[key] = { label: formatLabel(key, g), positive: 0, neutral: 0, negative: 0 }
    const s = c.sentiment_label ?? 'neutral'
    if (s === 'positive' || s === 'negative') groups[key][s]++
    else groups[key].neutral++
  }

  const points = Object.entries(groups).sort(([a], [b]) => a.localeCompare(b)).map(([, pt]) => pt)
