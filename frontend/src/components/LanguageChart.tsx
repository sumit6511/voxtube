...
const LANG_CONFIG: Record<string, { label: string; color: string }> = {
  nepali:  { label: 'Nepali',  color: '#10B981' },
  english: { label: 'English', color: '#378ADD' },
  neplish: { label: 'Neplish', color: '#F59E0B' },
}
...
export default function LanguageChart({ comments }: Props) {
  const counts = useMemo(() => {
    ...
    for (const comment of comments) {
      ...
      if (lang in c) c[lang]++; else c.neplish++
    }
    return c
  }, [comments])
  ...
  const chartData = Object.entries(LANG_CONFIG)
    .map(([key, cfg]) => ({ name: cfg.label, value: counts[key as keyof typeof counts], color: cfg.color }))
    .filter(d => d.value > 0)
        ...
        {Object.entries(LANG_CONFIG).map(([key, cfg]) => {
          ...
          return (
            <div key={key} className="text-center">
              <div className="font-mono text-xl font-medium" style={{ color: cfg.color }}>{count}</div>
