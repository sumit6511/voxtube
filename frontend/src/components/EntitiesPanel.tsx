...
export default function EntitiesPanel({ jobId }: Props) {
  ...
  if (!data && !loading && !error) return (
    <div className="card flex flex-col items-center justify-center py-12 gap-4">
      ...
      <button onClick={run} className="btn-primary text-sm px-5 py-2.5">Run Entity Extraction</button>
    </div>
  )
  ...
  if (loading) return (
    <div className="card flex items-center justify-center py-16 gap-3">
      ...
      <p className="font-mono text-sm text-gray-500">Extracting entities… (first run downloads model ~400MB)</p>
    </div>
  )
  ...
  if (error) return (
    <div className="card flex items-center gap-3 py-6">
      ...
      <button onClick={run} className="ml-auto text-xs font-mono text-gray-500 hover:text-white
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-4 items-center">
        ...
        {!model_available && (
          <div className="flex items-center gap-2 text-xs font-mono text-amber/80 bg-amber/5
                          border border-amber/20 rounded-lg px-3 py-2">
            <AlertTriangle size={12} /> Model unavailable — install torch and internet access required
          </div>
        )}
        <button onClick={run} className="ml-auto text-xs font-mono text-gray-500 hover:text-white
                                          border border-base-border px-3 py-1.5 rounded-lg transition-colors">
          ...
          return (
            <button key={cat} onClick={() => setFilter(cat)}
              className={`text-xs font-mono px-3 py-1.5 rounded-lg border transition-all
                ${filter === cat ? 'bg-amber text-gray-950 border-amber font-medium'
